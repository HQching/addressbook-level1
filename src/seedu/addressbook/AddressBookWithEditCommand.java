 ...
 * text file.
 **/
public class AddressBookWithEditCommand {
    ...
    private static final String COMMAND_EDIT_DESC = "Edits a person identified by the index number used in "
            + "the last find/list call.";
    private static final String COMMAND_EDIT_PARAMETERS = "INDEX " + PERSON_DATA_PREFIX_PHONE + "NEW_PHONE_NUMBER "
            + PERSON_DATA_PREFIX_EMAIL + "NEW_EMAIL";
    private static final String COMMAND_EDIT_EXAMPLE = COMMAND_EDIT_WORD + " 1 " + "p/98765432 e/johnd@gmail.com";
    ...
    private static ArrayList<String[]> getPersonsWithNameContainingAnyKeyword(Collection<String> keywords) {
        ...
        for (String[] person : getAllPersonsInAddressBook()) {
            ...
            findCaseInsensitiveMatch(keywords, matchedPersons, person, wordsInName);

        }
        return matchedPersons;
    }

    private static void findCaseInsensitiveMatch(Collection<String> keywords, final ArrayList<String[]> matchedPersons,
    ...
    private static String executeEditPerson(String commandArgs) {
        ...
        final Optional<String[]> decodeResult = decodePersonPropertyFromString(commandArgs);
        if (!decodeResult.isPresent()) {
            return getMessageForInvalidCommandInput(COMMAND_EDIT_WORD, getUsageInfoForEditCommand());
        }
        //check if index is valid
        final String[] targetPersonToEdit = decodeResult.get();
        final int targetVisibleIndex = Integer.parseInt(targetPersonToEdit[0]);
        if (!isDisplayIndexValidForLastPersonListingView(targetVisibleIndex)) {
            return MESSAGE_INVALID_PERSON_DISPLAYED_INDEX;
        }        
        
        String personToEditName = getPersonNameByLastVisibleIndex(targetVisibleIndex);
        int personToEditIdx = getPersonIndexInAddressBook(personToEditName);
        targetPersonToEdit[PERSON_DATA_INDEX_NAME] = personToEditName;
        ALL_PERSONS.remove(personToEditIdx);
        ALL_PERSONS.add(personToEditIdx, targetPersonToEdit);        
        return getMessageForSuccessfulEdit(targetPersonToEdit);
    }

    /**
     * Checks validity of edit person argument string's format.
     *
     * @param encoded  args string for the edit person command
     * @return whether the input args string is valid
     */
    private static Optional<String[]> decodePersonPropertyFromString(String encoded) {
        // check that we can extract the parts of a person from the encoded string
        if (!isPersonDataExtractableFrom(encoded)) {
            return Optional.empty();
        }
        final String[] decodedPersonProperty = makePersonFromData(extractNameFromPersonString(encoded),
                extractPhoneFromPersonString(encoded), extractEmailFromPersonString(encoded));
        // check that the constructed person is valid
        return isPersonDataValid(decodedPersonProperty) ? Optional.of(decodedPersonProperty) : Optional.empty();
    }
     ...
     * @param editedPerson successfully deleted
    ...
    /**
     * Displays all persons in the address book to the user; in alphabetical
     * order.
     *
     * @return feedback display message for the operation result
     */
    ...
    private static String executeSortAllPersonsInAddressBook() {
        ...
        class StringArrayComparator implements Comparator<String[]> {
            public int compare(String[] currString, String[] otherString) {
                return currString[0].toLowerCase().compareTo(otherString[0].toLowerCase());
            }
        }
        StringArrayComparator nameComp = new StringArrayComparator();
        Collections.sort(toBeDisplayed, nameComp);
        showToUser(toBeDisplayed);
        return getMessageForPersonsDisplayedSummary(toBeDisplayed);
    }
    ...
    private static String getPersonNameByLastVisibleIndex(int lastVisibleIndex) {
        return latestPersonListingView.get(lastVisibleIndex - DISPLAYED_INDEX_OFFSET)[PERSON_DATA_INDEX_NAME];
    }
    
    private static int getPersonIndexInAddressBook(String personToEditName) {
        for (String[] person : getAllPersonsInAddressBook()) {
            if (person[PERSON_DATA_INDEX_NAME].equals(personToEditName)) {
                return ALL_PERSONS.indexOf(person);
            }
        }
        return -1;
    }
    ...
    private static boolean isPersonNameValid(String name) {
        return name.matches("(\\w|\\s)+"); // name is nonempty mixture of
                                           // alphabets, numbers, and whitespace
    ...
    private static String getUsageInfoForAllCommands() {
        return getUsageInfoForAddCommand() + LS + getUsageInfoForFindCommand() + LS + getUsageInfoForListCommand() + LS
